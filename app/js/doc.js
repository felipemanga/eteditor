...
function Loader()
{
	...
	this.load = function load(url, name)
	{
		...
		if( ext.match(/\.jz$/) ){
			function getZip(){
				queueSize++;
				THIS.getURL( url, function(bin){
					if( bin[0] == "J" && bin[1] == "Z" )
						bin = bin.replace(/^JZ/, "PK");
					
					window.__ignoreErrors = true;
					var pending = [];
					var bork = false;
					function EH(e){
						bork = true;
					...
					window.addEventListener("error", EH, false);
					...
					var failCount = 0;
					while( failCount < pending.length ){
						var s = pending.shift();
						var el = MAR.create("script", { text: s.src });
						bork = false;
					...
					window.removeEventListener("error", EH, false);
					window.__ignoreErrors = false;
					
					popQueue(url);
				}, {binary:true});
			}
			need([{FQCN:"JSZip", URL:"js/jszip.min.js"}], getZip);
			popQueue(url + ".deps");
		}
		...
		else if( ext.match(/\.css$/) )
		{
			...
			if( window.matchMedia( q ).matches )
			{
				...
				se.addEventListener("load", function(){
					MAR.resolve(url.replace(/[\\/]+/g, '.'), null, se);
					popQueue(url);
				});
				se.href = url;
				se.rel="stylesheet";
...
function TEXT(txt)
{
	...
	while( m = txt.match(exp) )
		txt = txt.substr(0, m.index) + MAR.resolve(m[1], MAR.TEXT.context) + txt.substr(m.index + m[0].length);
...
TEXT.addTexts = function(language, textObj){
	...
	MAR.mergeTo( TEXT.languages[language], textObj );
};
...
var CLAZZ = window.CLAZZ = function CLAZZ( name, clazz ){
	"use strict";
	...
	while( cclazz ){
		...
		for( var k in cclazz )
		{
			if( k == "EXTENDS" || k == "ENUMERABLE" || k in desc ) continue;
			var v = cclazz[k];
			if( k == "STATIC" ){
				// MAR.mergeTo( statics, v );
				for( var sk in v )
				{
					if( !(sk in statics) )
						statics[sk] = v[sk];
	...
	var ret = function(){
		...
		{
			var superBackup = window.SUPER;
			if( clazz.CONSTRUCTOR.SUPER ) window.SUPER = clazz.CONSTRUCTOR.SUPER.bind(this);
	...
	MAR.mergeTo( ret, statics );
	ret.meta = metas;
	Object.seal(metas);
	
	ret.fullName = name;
	if( !name ) name = "$Anon" + MAR.getUID();
...
function Pool()
{
    ...
    this.add = function(obj, enableDirectMsg )
    {
        ...
        if( obj.__methods )
        {
        	var arr = obj.__methods;
        	if( !(obj.__methods instanceof Array) ) arr = Object.keys(obj.__methods);
            var l = arr.length;
            for( var i=0; i<l; ++i )
            {
                var m = arr[i];
    ...
    this.remove = function(obj)
    {
		...
		for( var k in (obj.__methods || obj) )
			this.mute(obj, k);
    };
...
var DOC = {
	...
	create: function()
	{
		...
		{
			var arg=arguments[i];
			var type = typeof arg;// MAR.typeOf(arg, Element);
			...
			case "object": MAR.mergeTo(prop, arg); break;
			case "array":  children = arg; break;
			case "Element": parent = arg; break;
			}
		}

		parent = parent || prop.parent || this.__ROOT__;
		...
		var f = {
			...
			dataset:MAR.mergeTo.bind(MAR, el.dataset),
			children:function(){},
			attr:function(a){
	...
	swapChildren:function( a, b ){
		...
		var ai = MAR.indexOf(a), bi = MAR.indexOf(b), t;
		if( bi < ai ){
			t = bi;
			bi = ai;
			ai = t;
	...
	index: function( root, obj, attach )
	{
		...
		if( attach ){
			for( var k in obj )
				DOC.attach( obj[k], attach[k], attach );
		}
		return obj;
	},
		...
		if( objk && listener ){
			if( Array.isArray(objk) ){
				...
				for( var e in listener ){
					...
					else objk.addEventListener(e, listener[e].bind(ctx));
				}
			}
		}
	},
	...
	appendFunction: function(object, name, func, pool){
		...
		{
			object[name] = func;
			if( object.__methods ) object.__methods[name] = object[name];
			...
			func.proxy = proxy;
			object[name] = func;
			if( object.__methods ) object.__methods[name] = func;
			if( pool ) pool.listen( object, name, false );
		}
	},
	...
	extend : function(){
		...
		for( var i=0; i<arguments.length; ++i ){
			var src = arguments[i];
			if(typeof src == "string") src = MAR.resolve(src);
			MAR.mergeToDeep(o, src);
		}
		return o;
	},
	...
	app:function(path, cfg){
		...
		document.addEventListener("DOMContentLoaded", function(){
			(new MAR.Loader()).load(file).start(function(){ MAR.resolve(path).NEW(); });
		});		
	}
};
